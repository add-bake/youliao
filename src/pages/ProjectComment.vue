    ...
    <div class="content">
      ...
      <ul>
        <li class="comment-item" v-for="item in list" :key="item.id">
          <div class="wrap btn-reply" v-if="item.replyFlag === '1'" :class="{special: item.officialUser}" @click="setReplyParams(item.criticizeUserPetname ? item.criticizeUserPetname : '创新访客', item.criticizeUserId, item.id)">
